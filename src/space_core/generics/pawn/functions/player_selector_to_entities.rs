...
use crate::space_core::{generics::{pawn::{events::NetConsoleCommands, resources::UsedNames}, networking::resources::ReliableServerMessage}};
    ...
    if player_selector == "*" {
        ...
        for (player_name, entity) in used_names.names.iter() {
            ...
            if precise_match == false {
                matcher = player_name.to_lowercase().contains(&player_selector.to_lowercase());
            } else {
        ...
        if found_one_match {
            ...
            let mut conflicting_message = "[color=#ff6600]Player selector \"".to_string() + &player_selector + " \" is not specific enough:\n";
