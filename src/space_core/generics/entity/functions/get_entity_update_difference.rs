...
use crate::space_core::generics::networking::resources::EntityUpdateData;
                ...
                for (new_entity_update_type, new_entity_update_data) in new_data_entity_updates {
                    ...
                    match old_data_entity_updates.get(new_entity_update_type) {
                        Some(old_entity_update_data) => {
                            ...
                            if !entity_data_is_matching(new_entity_update_data,old_entity_update_data) {
                                if !difference_data.contains_key(&new_node_path.to_string()) {
                                    difference_data.insert(new_node_path.to_string(),HashMap::new());
                                }
                                let difference_data_entity_updates = difference_data.get_mut(&new_node_path.to_string()).unwrap();
                                difference_data_entity_updates.insert(new_entity_update_type.clone(), new_entity_update_data.clone());
                            }
