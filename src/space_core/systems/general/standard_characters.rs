...
pub fn standard_characters(
    ...
    for (
        ...
        if pawn_component.just_spawned{
            pawn_component.just_spawned=false;

            let examine_tab = get_tab_action("examine").unwrap();

            pawn_component.tab_actions.push(examine_tab);
        }
