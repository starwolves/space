...
pub(crate) fn init_wall_low_curb_material(
    asset_server: Res<AssetServer>,
    mut materials: ResMut<Assets<StandardMaterial>>,
    mut res: ResMut<WallMaterials>,
) {
    let albedo_texture_handle =
        asset_server.load("gridmap/wall_low_curbed/wall_low_curbed_base.png");
    let metallic_roughness_texture_handle =
        asset_server.load("gridmap/wall_low_curbed/wall_low_curbed_metal_rough.png");
    ...
    res.low_curb_handle = material_handle;
}

pub(crate) fn init_wall_low_curb(
    ...
    if !is_server() || matches!(*app_mode, AppMode::Correction) {
        mesh_option = Some(meshes.wall_low_curbed.clone_weak());

        material_option = Some(mat.low_curb_handle.clone_weak());
    } else {
        mesh_option = None;
        material_option = None;
    }
    init.properties.push(TileProperties {
        name_id: CellTypeName("wall_low_curb".to_string()),
        name: RichName {
