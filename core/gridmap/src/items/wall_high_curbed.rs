...
pub(crate) fn init_wall_high_curb_material(
    asset_server: Res<AssetServer>,
    mut materials: ResMut<Assets<StandardMaterial>>,
    mut res: ResMut<WallMaterials>,
) {
    let albedo_texture_handle =
        asset_server.load("gridmap/wall_high_curbed/wall_curb_high_base.png");
    let metallic_roughness_texture_handle =
        asset_server.load("gridmap/wall_high_curbed/wall_curb_high_metal_rough.png");
    ...
    res.high_curb_handle = material_handle;
}

pub(crate) fn init_wall_high_curb(
    ...
    if !is_server() || matches!(*app_mode, AppMode::Correction) {
        mesh_option = Some(meshes.wall_high_curbed.clone_weak());

        material_option = Some(mat.high_curb_handle.clone_weak());
    } else {
        mesh_option = None;
        material_option = None;
    }
    init.properties.push(TileProperties {
        name_id: CellTypeName("wall_high_curb".to_string()),
        name: RichName {
            name: "curb wall".to_string(),
            n: true,
            the: false,
        },
        description: "A curb wall tile.".to_string(),
        constructable: true,
        mesh_option,
